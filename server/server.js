...
const { getEventDateString, isSameEventDate } = require('./utils/dateMatching');
...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      const checkInDate = getEventDateString(createdAt) || 'unknown';
      const matchesDate = isSameEventDate(createdAt, eventDate);
      ...
      // Filter by security code match, active status, and event date.
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Clear notifications only when switching sessions so same-session relaunches do not drop pickups.
    const beforeCount = activeNotifications.length;
    const clearedNotifications = beforeCount > 0 && shouldClearNotifications(globalBillboardState, eventId, eventDate);
    ...
    res.json({ 
      ...
      globalBillboardState,
      notificationsCleared: clearedNotifications ? beforeCount : 0
    });
  } catch (error) {
