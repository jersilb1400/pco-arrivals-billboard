...
const checkInCache = createCheckInCache({ timeoutMs: 30000 });
...
function updateCheckInCache(cacheKey, data) {
  checkInCache.update(cacheKey, data);
}
  ...
  try {
    ...
    if (securityCodes && securityCodes.length > 0) {
      try {
        ...
        const securityCodesCacheKey = buildCheckInCacheKey('security-codes', eventId, 'all');
...
setInterval(async () => {
  try {
    // Keep pickup requests visible until PCO confirms checkout.
    // A time-based expiry can silently remove children who are still waiting.
    if (activeNotifications.length > 0) {
      const checkInIds = activeNotifications.map(n => n.checkInId);
...
app.get('/api/location-status', async (req, res) => {
  try {
    ...
    if (cachedData && cachedData.data && cachedData.included) {
      console.log(`[DEBUG] Location-status: Using cached data for ${cacheKey}`);
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Clear notifications only when switching event context; relaunching the same
    // event/date must preserve in-flight pickup requests.
    const beforeCount = activeNotifications.length;
    const clearNotifications = shouldClearNotifications(globalBillboardState.activeBillboard, eventId, eventDate);
    if (beforeCount > 0 && clearNotifications) {
      activeNotifications.length = 0;
