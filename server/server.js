...
app.post('/api/security-codes', async (req, res) => {
  try {
    ...
    if (securityCodes && securityCodes.length > 0) {
      const securityCodesCacheKey = buildCheckInCacheKey(eventId, {
      ...
      try {
        ...
        const cachedData = checkInCache.get(securityCodesCacheKey);
        ...
        if (cachedData) {
          ...
          // Cache the data
          checkInCache.set(securityCodesCacheKey, { data: allCheckIns, included });
        }
...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      const matchesDate = isCheckInOnEventDate(checkInDate, eventDate);

      console.log(`[SECURITY CODE] Check-in ${checkIn.id}: code=${checkIn.attributes.security_code} (normalized: ${checkInSecurityCode}), matchesCode=${matchesSecurityCode}, active=${isActive}, checkInDate=${checkInDate || 'unknown'}, eventDate=${eventDate}, matchesDate=${matchesDate}`);
...
app.get('/api/location-status', async (req, res) => {
  try {
    ...
    // Check cache first
    const locationStatusCacheKey = buildCheckInCacheKey(eventId, {
      endpoint: 'location-status',
      date: date,
      include: 'person,locations,checked_in_at'
    });
    const cachedData = checkInCache.get(locationStatusCacheKey);
    ...
    if (allCheckIns.length > 0) {
      checkInCache.set(locationStatusCacheKey, { data: allCheckIns, included: allIncluded });
    }
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Clear notifications from past events when starting a new event/date.
    const beforeCount = activeNotifications.length;
