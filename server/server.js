...
const { checkInCacheKey, createCheckInCache } = require('./utils/checkInCache');
...
let checkInCache = createCheckInCache(30000);
...
function getCachedCheckInData(keyParts) {
  return checkInCache.get(keyParts);
}

// Function to update check-in cache
function updateCheckInCache(keyParts, data) {
  checkInCache.set(keyParts, data);
}
...
app.post('/api/security-codes', async (req, res) => {
  try {
    ...
    if (securityCodes && securityCodes.length > 0) {
      const securityCodesCacheKey = checkInCacheKey({ eventId, scope: 'security-codes', include: 'person,household' });
      try {
        // Check cache first
...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      console.log(`[SECURITY CODE] Check-in ${checkIn.id}: code=${checkIn.attributes.security_code} (normalized: ${checkInSecurityCode}), matchesCode=${matchesSecurityCode}, active=${isActive}, createdAt=${createdAt || 'unknown'}, eventDate=${eventDate}, matchesDate=${matchesDate}`);
...
app.get('/api/location-status', async (req, res) => {
  try {
    ...
    // Check cache first
    const cacheKey = checkInCacheKey({ eventId, scope: 'location-status', date, include: 'person,locations,checked_in_at' });
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    const shouldClearActiveNotifications = shouldClearNotifications(globalBillboardState.activeBillboard, eventId, eventDate);
    const notificationsCleared = shouldClearActiveNotifications ? activeNotifications.length : 0;
    if (shouldClearActiveNotifications && notificationsCleared > 0) {
      activeNotifications = [];
