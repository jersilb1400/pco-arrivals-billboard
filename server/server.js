...
const { checkInMatchesEventDate, getCheckInEventDate } = require('./utils/dateMatching');
...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      const checkInDate = getCheckInEventDate(checkIn.attributes.created_at) || 'unknown';
      const matchesDate = checkInMatchesEventDate(checkIn.attributes.created_at, eventDate);
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Clear notifications from past sessions only when changing event or date.
    const beforeCount = activeNotifications.length;
    const clearNotifications = shouldClearNotifications(currentBillboard, {
      eventId,
      eventDate: nextEventDate
    });
    if (beforeCount > 0 && clearNotifications) {
      activeNotifications.length = 0;
      console.log(`Server: Cleared ${beforeCount} notifications from previous sessions`);
    }
