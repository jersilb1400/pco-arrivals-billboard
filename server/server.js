...
app.get('/api/active-notifications', async (req, res) => {
  try {
    ...
    // Filter notifications by event and date to prevent showing previous sessions.
    const notificationScope = eventId && eventDate
      ? { eventId, eventDate }
      : globalBillboardState.activeBillboard;
    ...
    if (notificationScope?.eventId && notificationScope?.eventDate) {
      console.log(`[DEBUG] Filtered to ${filteredNotifications.length} notifications for event ${notificationScope.eventId} on date ${notificationScope.eventDate} (strict matching)`);
    ...
    if (filteredNotifications.length > 0) {
      ...
      try {
        ...
        if (checkedOutIds.length > 0) {
          ...
          filteredNotifications = pruneCheckedOutNotifications(filteredNotifications, checkedOutIds);
...
setInterval(async () => {
  try {
    ...
    // Pickup requests must remain visible until checkout is confirmed.
    if (activeNotifications.length > 0) {
      const checkInIds = activeNotifications.map(n => n.checkInId);
...
app.get('/api/location-status', async (req, res) => {
  try {
    ...
    // Check cache first
    const locationStatusInclude = 'person,locations,checked_in_at';
    const cacheKey = buildCheckInCacheKey({
      route: 'location-status',
      eventId,
      date,
      include: locationStatusInclude
    });
    ...
    while (nextPage && pageCount < maxPages) {
      ...
      nextPage = links && links.next ? links.next : null;
        ...
        if (apiError.response?.status === 429) {
          console.log(`[DEBUG] Location-status: Rate limited, returning partial data without caching`);
    ...
    if (allCheckIns.length > 0 && completedAllPages) {
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    const resolvedEventDate = resolveNextEventDate(eventDate, currentBillboard, eventId);
    ...
    // Clear only notifications from past sessions when starting a different event/date.
    const beforeCount = activeNotifications.length;
    if (beforeCount > 0 && shouldClearNotifications(currentBillboard, { eventId, eventDate: resolvedEventDate })) {
      activeNotifications = filterNotificationsForScope(activeNotifications, {
        eventId,
        eventDate: resolvedEventDate
      });
      console.log(`Server: Cleared ${beforeCount - activeNotifications.length} notifications from previous events`);
    }
    const notificationsCleared = beforeCount - activeNotifications.length;
    
    updateGlobalBillboardState(eventId, eventName, securityCodes || [], resolvedEventDate, userId, userName, colorsForState, stationsForState, iconsForState);
