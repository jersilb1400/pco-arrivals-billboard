...
const { getDateInTimeZone, isCheckInOnEventDate } = require('./utils/dateMatching');
...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      const checkInDate = getDateInTimeZone(checkIn.attributes.created_at) || 'unknown';
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Clear notifications only when switching sessions so same-event admin saves do not drop pickup requests.
    const beforeCount = activeNotifications.length;
    const notificationsCleared = shouldClearNotifications(globalBillboardState, { eventId, eventDate });
    if (notificationsCleared && beforeCount > 0) {
      activeNotifications.length = 0;
    ...
    res.json({ 
      ...
      globalBillboardState,
      notificationsCleared: notificationsCleared ? beforeCount : 0
    });
  } catch (error) {
