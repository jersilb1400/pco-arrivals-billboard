...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Only clear notifications when switching to a different event/date session.
    const previousBillboard = globalBillboardState.activeBillboard;
    const shouldClearForNewSession = shouldClearNotifications(previousBillboard, eventId, eventDate);
    const beforeCount = activeNotifications.length;
    if (shouldClearForNewSession && beforeCount > 0) {
      activeNotifications.length = 0;
    ...
    res.json({ 
      ...
      globalBillboardState,
      notificationsCleared: shouldClearForNewSession ? beforeCount : 0
    });
  } catch (error) {
