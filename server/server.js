...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    // Filter by security code (case-insensitive), active status, and the event calendar date.
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      const matchesDate = isMatchingEventDate(checkIn.attributes.created_at, eventDate);
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Clear only when the active event session changes; same-session saves must not drop pickups.
    const beforeCount = activeNotifications.length;
