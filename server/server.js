...
const { checkInMatchesEventDate, getCheckInDateCandidates } = require('./utils/dateMatching');
...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    // Filter by security code (case-insensitive), active status, and selected event date.
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      const checkInDateCandidates = getCheckInDateCandidates(createdAt);
      const checkInDate = checkInDateCandidates[0] || 'unknown';
      const matchesDate = checkInMatchesEventDate(createdAt, eventDate);
      
      console.log(`[SECURITY CODE] Check-in ${checkIn.id}: code=${checkIn.attributes.security_code} (normalized: ${checkInSecurityCode}), matchesCode=${matchesSecurityCode}, active=${isActive}, checkInDate=${checkInDate}, dateCandidates=${checkInDateCandidates.join('|') || 'none'}, eventDate=${eventDate}, matchesDate=${matchesDate}`);
      
      
      // Filter by security code match, active status, and strict event date.
      return matchesSecurityCode && isActive && matchesDate;
    });

    console.log(`[SECURITY CODE] Found ${activeCheckIns.length} active check-ins (matching by code, active status, and selected event date)`);
