...
const {
  ...
  getCachedCheckInData: readCheckInCache,
  updateCheckInCache: writeCheckInCache
...
// Cache check-ins per event/date/include shape to avoid cross-session data reuse.
let checkInCache = createEmptyCheckInCache();
...
function updateGlobalBillboardState(eventId, eventName, securityCodes, eventDate, userId, userName, locationColors, stationColors, stationIcons) {
  ...
  const sameEvent = currentBillboard && String(currentBillboard.eventId) === String(eventId);
  globalBillboardState = {
    activeBillboard: {
      ...
      eventDate,
      locationColors: locationColors || (sameEvent ? currentBillboard.locationColors : {}) || {},
      stationColors: stationColors || (sameEvent ? currentBillboard.stationColors : {}) || {},
      stationIcons: stationIcons || (sameEvent ? currentBillboard.stationIcons : {}) || {}
    },
...
function getCachedCheckInData(eventId, options) {
  return readCheckInCache(checkInCache, eventId, options);
}

// Function to update check-in cache
function updateCheckInCache(eventId, data, options) {
  checkInCache = writeCheckInCache(checkInCache, eventId, data, options);
}
  ...
  try {
    ...
    if (securityCodes && securityCodes.length > 0) {
      try {
        ...
        const securityCodesCacheOptions = { includeKey: 'security-codes' };
        const cachedData = getCachedCheckInData(eventId, securityCodesCacheOptions);
        ...
        if (cachedData) {
          ...
          // Cache the data
          updateCheckInCache(eventId, { data: allCheckIns, included }, securityCodesCacheOptions);
        }
        ...
        if (apiError.response?.status === 429) {
          ...
          const cachedData = getCachedCheckInData(eventId, { includeKey: 'security-codes' });
          if (cachedData) {
            // Process cached data
...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    // Filter by security code (case-insensitive), active status, and exact event date.
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      const checkInDate = getDateStringForEventTimeZone(checkIn.attributes.created_at, EVENT_TIME_ZONE) || 'unknown';
    ...
    console.log(`[SECURITY CODE] Found ${activeCheckIns.length} active check-ins (matching by code, active status, and event date in ${EVENT_TIME_ZONE})`);
...
app.get('/api/location-status', async (req, res) => {
  try {
    ...
    // Check cache first. Location status needs date-scoped data with Location resources.
    const locationStatusCacheOptions = { date, includeKey: 'location-status' };
    const cachedData = getCachedCheckInData(eventId, locationStatusCacheOptions);
    ...
    if (allCheckIns.length > 0) {
      updateCheckInCache(eventId, { data: allCheckIns, included: allIncluded }, locationStatusCacheOptions);
    }
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    const beforeCount = activeNotifications.length;
    const shouldClear = shouldClearNotifications(currentBillboard, eventId, nextEventDate);
    if (beforeCount > 0 && shouldClear) {
      activeNotifications.length = 0;
    ...
    res.json({ 
      ...
      globalBillboardState,
      notificationsCleared: shouldClear ? beforeCount : 0
    });
  } catch (error) {
