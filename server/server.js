...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Re-saving the same active session must preserve in-flight pickup requests.
    const beforeCount = activeNotifications.length;
    ...
    if (shouldClear && beforeCount > 0) {
      ...
      console.log(`Server: Preserved ${beforeCount} active notifications for same event session`);
    }
