...
const checkInCache = createCheckInCache(30000);
  ...
  try {
    ...
    if (securityCodes && securityCodes.length > 0) {
      try {
        ...
        const securityCodesCacheKey = `security-codes:${eventId}:${eventDate || 'all'}:person-household`;
        const cachedData = getCachedCheckInData(securityCodesCacheKey);
        
        if (cachedData) {
          console.log('Using cached check-in data for key:', securityCodesCacheKey);
...
app.get('/api/global-billboard', async (req, res) => {
  try {
    ...
    if (req.isPublicRequest) {
      return res.json(sanitizeGlobalBillboardForPublic(responseData));
    }
...
app.post('/api/security-code-entry', async (req, res) => {
  try {
    ...
    if (req.isPublicRequest && !publicRequestMatchesActiveBillboard({ eventId, eventDate }, globalBillboardState)) {
      return res.status(403).json({
        success: false,
        message: 'Security code entry is not available for this event.'
      });
    }
    ...
    // Filter by security code (case-insensitive), active status, and the event-local date.
    const activeCheckIns = allCheckIns.filter(checkIn => {
      ...
      const matchesDate = isSameEventDate(
        checkIn.attributes.created_at,
        eventDate,
        process.env.EVENT_TIME_ZONE || 'America/Chicago'
      );
      
      console.log(`[SECURITY CODE] Check-in ${checkIn.id}: code=${checkIn.attributes.security_code} (normalized: ${checkInSecurityCode}), matchesCode=${matchesSecurityCode}, active=${isActive}, checkInTime=${checkIn.attributes.created_at}, eventDate=${eventDate}, matchesDate=${matchesDate}`);
      
      
      // Filter by security code match, active status, and the exact event-local date.
  ...
  try {
    ...
    if (req.isPublicRequest) {
      ...
      if (eventId || eventDate) {
        if (!publicRequestMatchesActiveBillboard({ eventId, eventDate }, globalBillboardState)) {
          return res.status(403).json({ error: 'Notifications are not available for this event.' });
        }
      } else {
        scopedEventId = activeScope.eventId;
        scopedEventDate = activeScope.eventDate;
      }
    }

    if (scopedEventId && scopedEventDate) {
      console.log(`[DEBUG] Filtering by event: ${scopedEventId}, date: ${scopedEventDate}`);
    }
    ...
    if (scopedEventId && scopedEventDate) {
      ...
      console.log(`[DEBUG] Filtered to ${filteredNotifications.length} notifications for event ${scopedEventId} on date ${scopedEventDate} (strict matching)`);
...
app.get('/api/location-status', async (req, res) => {
  try {
    ...
    if (req.isPublicRequest) {
      ...
      if (String(eventId) !== String(activeScope.eventId)) {
        return res.status(403).json({ error: 'Location status is not available for this event.' });
      }

      if (date && String(date) !== String(activeScope.eventDate)) {
        return res.status(403).json({ error: 'Location status is not available for this date.' });
      }

      effectiveDate = date || activeScope.eventDate;
    }

    // Check cache first
    const cacheKey = `location-status:${eventId}:${effectiveDate || 'all'}:person-locations-stations`;
    ...
    if (effectiveDate) {
      url += `&where[created_at]=${effectiveDate}`;
    }
...
app.post('/api/set-global-billboard', async (req, res) => {
  try {
    ...
    // Clear stale notifications only when moving to a different event/date session.
    const beforeCount = activeNotifications.length;
    let notificationsCleared = 0;
    if (beforeCount > 0 && shouldClearNotifications(currentBillboard, eventId, nextEventDate)) {
      activeNotifications.length = 0;
      notificationsCleared = beforeCount;
